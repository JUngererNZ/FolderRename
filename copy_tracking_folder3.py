...
import sys
import json
import subprocess  # <-- Used to call the diff scripts
...
def main():
    ...
    vessel_script = os.path.join(script_dir, "vessel_tracker_diff.py")

    if not os.path.exists(vessel_script):
        print(f"[WARNING] Could not find {vessel_script}. Skipping vessel diff step.")
    else:
        try:
            result = subprocess.run(
                [sys.executable, vessel_script],
    ...
    if not os.path.exists(quote_script):
        print(f"[FATAL] Could not find dependent script: {quote_script}")
        sys.exit(1)
    ...
    try:
        ...
        if quote_result.returncode == 0:
            print("--- Quote Register diff completed successfully. ---")
        else:
            print(f"[FATAL] Quote Register diff failed with exit code: {quote_result.returncode}")
            sys.exit(1)
